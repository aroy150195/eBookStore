 ...
 *
 * A base class for creating dialog fragments that integrate Android's [ViewDataBinding]
 * with a [BaseViewModel].
 *
 * This abstract class extends [BaseViewModelDialogFragment] and provides a shared
 * foundation for dialog fragments that use the Data Binding library to bind UI
 * components directly to observable data exposed by a ViewModel. It ensures that
 * both the binding and ViewModel lifecycles are properly tied to the dialog fragment.
 *
 ...
 *
 * @param T the type of [ViewDataBinding] associated with this dialog's layout.
 *          This allows subclasses to access the generated binding class directly.
 * @param VM the type of [BaseViewModel] associated with this dialog fragment.
 *           The ViewModel is expected to manage UI state and handle events
 *           specific to the dialog's screen.
 *
 * Example usage:
 * ```
 * class ConfirmationDialog :
 *     BaseDataBindingDialogFragment<DialogConfirmationBinding, ConfirmationViewModel>() {
 *
 ...
 * ```
 */
abstract class BaseViewDataBindingDialogFragment<T : ViewDataBinding, VM : BaseViewModel<*, *>> :
    BaseViewModelDialogFragment<VM>() {
